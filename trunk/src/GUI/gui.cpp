...
// used to disable both random and zig zag buttons until after save image has been selected
int count = 0;
    ...
    if( event.type == SDL_MOUSEBUTTONDOWN )
    {
        ...
        if( event.button.button == SDL_BUTTON_LEFT )
        {
            ...
            if( ( x > box.x ) && ( x < box.x + box.w ) && ( y > box.y ) && ( y < box.y + box.h ) )
            {
				...
				//Take Picture
				// boolean ended is used to disable all the buttons after search has been terminated
				if (( x > 30) && (x < 30 + box.w)  && (y > 120) && (y < 120 + box.h) && count == 0)
				{
					system("start /MIN control.exe");
					Sleep(500);
					// this is the code that will make the image pop up
					// SDL_Rect *Init(int XPos, int YPos, int Width, int Height)
					// screen width: 640
					// screen height: 480
					int x = 290;
					int y = 150;
					myRect = Init(0, 0, 250, 250);
			//		picture = load_image("closer.jpg");
					
					picture = load_image("control.jpg");
					apply_surface(x, y, picture, screen, myRect);
					count++;
					delete myRect; // not sure what this will do, but it said to delete it just in case
				}
				
				//Zig zag
				if((x > 30) && ( x< 30 + box.w) && (y > 220) && (y < 220 + box.h) && count == 1)
				{
					count++;
					system("start /MIN zigzag.exe");
				}
				//Rand
				if((x > 30) && ( x< 30 + box.w) && (y > 320) && (y < 320 + box.h)  && count == 1)
				{
					count++;
